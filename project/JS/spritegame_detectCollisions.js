...
function leavesBottom(player, item) {
    let playeroff = player.offsetTop;
    let itemheight = (item.clientHeight - player.clientHeight)
    return playeroff > itemheight
}

function leavesLeft(player) {
    let playeroff = player.offsetLeft;
    return playeroff < 0
}

function leavesRight(player, item) {
    let playeroff = player.offsetLeft;
    let itemwidth = (item.clientWidth - player.clientWidth)
    return playeroff > itemwidth
}

function leavesTop(player) {
    let playeroff = player.offsetTop;
    return playeroff < 0
}